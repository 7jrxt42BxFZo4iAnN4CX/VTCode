...
- Anti-blind editing is session-scoped: once successful mutations require verification, a blocked turn and later `continue` retain the gate. Only a successful verification-classified command clears it; reads, diffs, failed checks, and ordinary stall recovery do not.
...
- The assistant text-response cap is a blocked safety stop, not successful completion. If only substantive commentary exists at the cap, promote the latest commentary to the final history phase and reuse the existing renderer/stream flags so the user and canonical `ThreadEvent` stream receive exactly one preserved response; use the deterministic fallback only when no substantive response exists.
