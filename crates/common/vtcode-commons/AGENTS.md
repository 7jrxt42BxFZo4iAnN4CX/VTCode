...
- `env_lock/` is macOS-specific env mutex — used by `vtcode` binary, not by library crates; `sanitizer::StreamingSecretRedactor` carries a bounded suffix across pipe/PTY chunks, so use it for streamed spool writes.
