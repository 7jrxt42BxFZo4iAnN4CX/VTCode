...
- `env_lock/` is macOS-specific env mutex — used by `vtcode` binary, not by library crates; `startup_trace/` is an opt-in pre-tracing phase recorder; `sanitizer::StreamingSecretRedactor` carries a bounded suffix across pipe/PTY chunks, so use it for streamed spool writes.
