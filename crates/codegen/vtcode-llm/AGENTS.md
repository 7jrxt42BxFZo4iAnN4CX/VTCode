...
## Architecture Notes
- **Canonical home** for all provider code. Core's `llm/` is a thin re-export layer + factory/CGP. Merge Gateway defaults to native `/v1/responses`; an explicit `/v1/openai` base selects legacy Chat Completions. Native snapshots stay buffered until the terminal frame so `fallback_restart` can discard stale output. Route-specific reasoning controls must not be inferred as generic provider fields.
