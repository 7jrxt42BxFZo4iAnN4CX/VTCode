...
## Architecture Notes
- **Canonical home** for all provider code. Core's `llm/` is a thin re-export layer + factory/CGP. Merge Gateway uses the OpenAI-compatible Chat Completions shell only; route-specific reasoning controls must not be inferred as generic provider fields.
