...
- `agent.ui_surface` defaults to `inline` so interactive plan/interview HITL overlays work without configuration; `auto` and `alternate` remain explicit overrides.
- Use `api_keys::get_api_key_with_mode` and `provider_credential_detail_with_mode` when a loaded config is available; the compatibility wrappers use the platform default storage mode.
