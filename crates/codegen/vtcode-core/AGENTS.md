...
- Planning persistence: the dedicated plan agent can enter an active planning session before `start_planning` creates an artifact. `persist_plan_draft` must lazily allocate a workspace-local `.vtcode/plans/<timestamp>.md` before emitting approval events; do not restore the hard failure for an active session with no current plan path.
