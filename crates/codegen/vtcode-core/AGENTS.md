...
- Async plugin, skill, file-tool, planning, persistent-memory, durable-scheduler, prompt-resource cache-miss, context-reset manifest, and trajectory setup paths must use Tokio filesystem APIs or `spawn_blocking` for recursive/synchronous scans, record loading, claim files, persistence, or Git work.
- `SessionToolCatalog` projection caches are private and lazy per entry/documentation mode; rebuild the catalog when registrations change.
