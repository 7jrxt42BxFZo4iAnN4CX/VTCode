...
- **Model providers**: 24+ LLM providers including Anthropic, OpenAI, Gemini, OpenRouter, NVIDIA NIM, **local inference via Ollama, LM Studio, and llama.cpp** (managed with the `/local` command), and more
...
- [**Providers**](./docs/providers/PROVIDER_GUIDES.md): setup guides for all 24 providers
...
VT Code supports 24+ LLM providers out of the box, plus any OpenAI-compatible API via `[[custom_providers]]`. Custom providers can declare their model context window in tokens with `context_window`; when omitted they default to 128K. The configured value drives the context size shown in the UI, automatic compaction thresholds, and preflight token checks.
